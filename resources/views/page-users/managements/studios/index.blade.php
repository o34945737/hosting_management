<x-layouts.app>
    <x-slot name="toolbar">
        <x-layouts.toolbar title="Studio" />
    </x-slot>
            ...
            <div class="modal-dialog modal-dialog-centered mw-650px">
                <div class="modal-content rounded">
                    ...
                    <div class="modal-body scroll-y px-10 px-lg-15 pt-0 pb-15">
                        ...
                        <form id="studios_form" method="POST" action="{{ route('tenant.studios.store') }}">
                            @csrf

                            <div class="d-flex flex-column mb-6 fv-row">
                                <label class="fs-6 fw-semibold mb-2">Name <span class="required"></span></label>
                                <input type="text" class="form-control form-control-solid" placeholder="Studio" name="name" autocomplete="off">
                                <div class="text-danger small mt-1" data-error="name"></div>
                            </div>

                            <div class="d-flex flex-column mb-6 fv-row">
                                <label class="fs-6 fw-semibold mb-2">Location <span class="required"></span></label>
                                <input type="text" class="form-control form-control-solid" placeholder="Location" name="location" autocomplete="off">
                                <div class="text-danger small mt-1" data-error="location"></div>
                            </div>
                            ...
                            <div class="text-center mt-8">
                                ...
                                <button type="submit" id="studios_submit" class="btn btn-primary">
...
$(function () {
  ...
  // DataTable (Studios)
  const dt = $('#studiosTable').DataTable({
    processing: true,
    serverSide: true,
    responsive: true,
    ajax: "{{ route('tenant.studios.data') }}",
    columns: [
      { data: 'DT_RowIndex', orderable:false, searchable:false, className:'text-center', width:'50px' },
      { data: 'name', name: 'name' },
      { data: 'location', name: 'location' },
      // ✅ biar search description jalan (server already handles it)
  ...
  const $form   = $('#studios_form');
  const $submit = $('#studios_submit');
  ...
  function resetCreate() {
    ...
    $form.attr('action', "{{ route('tenant.studios.store') }}");
  ...
  function setEdit(id, data) {
    ...
    setMethodSpoof('PUT');
    $form.attr('action', "{{ route('tenant.studios.update', ':id') }}".replace(':id', id));
  ...
  // OPEN EDIT
  $('#studiosTable').on('click', '.btn-edit', function () {
    const id = $(this).data('id');
    const showUrl = "{{ route('tenant.studios.show', ':id') }}".replace(':id', id);
  ...
  // DELETE
  $('#studiosTable').on('click', '.btn-delete', function () {
    const id = $(this).data('id');
    const deleteUrl = "{{ route('tenant.studios.destroy', ':id') }}".replace(':id', id);
